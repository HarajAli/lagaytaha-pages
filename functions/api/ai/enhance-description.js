...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // ── 7) استدعاء DeepSeek ─────────────────────────────────
    const systemPrompt = `أنت مساعد متخصص في كتابة إعلانات الحراج في اليمن.
المستخدم كتب الوصف التالي:
"${description}"

الفئة: ${category || 'غير محددة'}

اكتب نسخة محسّنة من الوصف بنفس اللغة (عربية يمنية بسيطة)، بحيث:
- تذكر المواصفات المذكورة فقط بوضوح، بدون اختراع أي معلومة جديدة (مواصفات، حالة، ضمان، سعر، موقع، سبب بيع).
- دورك إعادة صياغة وتنظيم وتحسين فقط.
- لا تتجاوز 100 كلمة.

أعد فقط الوصف المحسّن، بدون مقدمات أو شرح.`;
